...
El workflow `.github/workflows/ci.yml` construye y sube `vigilay-web` a GHCR en cada `push` a `main`. Si se configuran la variable de repositorio `BUNNYNET_APP_ID` y el secreto `BUNNYNET_API_KEY`, el job `deploy-web-magic-container` actualiza automáticamente el contenedor `web` al SHA recién publicado. El registro GHCR debe estar agregado previamente en Magic Containers.
